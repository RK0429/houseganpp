...
# import PIL # unused
# import svgwrite # unused
import torch

# import torchvision.transforms as T # unused
import webcolors  # type: ignore
from PIL import (  # Removed unused: ImageColor, ImageFilter, ImageFont, ImageOps
    Image,
    ImageDraw,
)

# from pygraphviz import * # unused star import
# from skimage.transform import resize as imresize # unused
# from torch.utils.data import Dataset # unused
# from torchvision.utils import save_image # unused

# from misc.intersections import doIntersect # unused
...
def selectRandomNodes(nd_to_sample, batch_size):
    ...
    for k in range(batch_size):
        ...
        ).cuda()  # torch.tensor(list(range(rooms_num))).long().cuda() # Fixed comment style
...
def selectNodesTypes(nd_to_sample, batch_size, nds):
    all_types, fixed_nodes, shift = (  # removed unused fixed_rooms_num
        [ROOM_CLASS[k] - 1 for k in ROOM_CLASS],
        # [], # removed corresponding list for fixed_rooms_num
        [],
        0,
    )
    for k in range(batch_size):
        rooms = np.where(nd_to_sample == k)
        # rooms_num = np.array(rooms).shape[-1] # unused
        ...
        # existing_types_str = '-'.join([str(i) for i in _types])
        # selected_types_str = '-'.join([str(i) for i in _t])
        # fixed_rooms_str = '-'.join([str(i) for i in fixed_rooms])
        # print(f' existing types: {existing_types_str} \\n '
        #       f'selected types: {selected_types_str} \\n '
        #       f'fixed rooms {fixed_rooms_str}') # Shortened long line
...
def fix_nodes(prev_mks, ind_fixed_nodes):
    ...
    # Set non fixed masks to -1.0 # Fixed comment style
    given_masks[ind_not_fixed_nodes.long()] = -1.0
    given_masks = given_masks.unsqueeze(1)
    # Add channel to indicate given nodes # Fixed comment style
...
def combine_images(samples_batch, nodes_batch, edges_batch, nd_to_sample, ed_to_sample):
    ...
    for b in range(batch_size):
        ...
        # inds_ed = np.where(ed_to_sample == b) # unused
        sps = samples_batch[inds_nd]
        nds = nodes_batch[inds_nd]
        # eds = edges_batch[inds_ed] # unused
...
def draw_graph(g_true):
    ...
    # rgb_arr = pad_im(rgb_im).convert("RGBA") # unused
...
def estimate_graph(masks, nodes, G_gt):
    ...
    for k in doors_inds:
        for room_idx in rooms_inds:  # Renamed ambiguous variable 'l'
            if k > room_idx:
                m1, m2 = masks[k], masks[room_idx]
                m1[m1 > 0] = 1.0
                m1[m1 <= 0] = 0.0
                m2[m2 > 0] = 1.0
                m2[m2 <= 0] = 0.0
                ...
                if iou > 0 and iou < 0.2:
                    doors_rooms_map[k].append((room_idx, iou))
